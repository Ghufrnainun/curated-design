...
The deployed catalog is machine-readable from the live site, not only this
repo. Agents pointed at the site should fetch:

- `https://design.ghuf.app/llms.txt` — category index (standard llms.txt)
- `https://design.ghuf.app/llms-full.txt` — every resource: `[Name](url) — note`

These are generated by `scripts/build.mjs` into `public/` on every build.
Never edit them by hand; edit `resources/*.md` instead.
